...
function dominantDirection(text) {
  ...
  let countMap = {};
  for ( let script of scripts ) {
    let direction = SCRIPTS[SCRIPTS.findIndex( x => x["name"] == script["name"])]["direction"];
    if ( countMap[direction] == undefined ) {
      countMap[direction] = script["count"];
    } else {
      countMap[direction] += script["count"];
    }
  }
    let max = Object.keys(countMap).reduce((a, b) => countMap[a] > countMap[b] ? a : b);
    return  max;
  }
